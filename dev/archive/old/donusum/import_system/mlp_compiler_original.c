...
Token* getNextToken() {
    ...
    if (current_char == '.') {
        current_position++;
        printf("DEBUG: Lexer found DOT. TOKEN_DOT=%d\n", TOKEN_DOT);
...
Token* check_keyword(const char* word) {
    // DEBUG
    // printf("DEBUG: Checking keyword '%s'\n", word);
...
void register_parse_var(const char* name, const char* type) {
    ...
    // DEBUG
    // printf("DEBUG: Registering var '%s' of type '%s'\n", name, type);
...
const char* get_parse_var_type(const char* name) {
    for (int i = parse_var_count - 1; i >= 0; i--) {
        if (strcmp(parse_vars[i].name, name) == 0) {
            // DEBUG
            // printf("DEBUG: Found var '%s' of type '%s'\n", name, parse_vars[i].type);
            return parse_vars[i].type;
        }
    }
    // DEBUG
    // printf("DEBUG: Var '%s' not found in registry\n", name);
    return NULL;
}
...
void consume(TokenType expected_type) {
    ...
    // DEBUG PRINT
    // printf("DEBUG: Consuming %s (Value: %s)\n", getTokenTypeName(current_token->type), current_token->value ? current_token->value : "NULL");
...
ASTNode* birincil() {
    ...
    if (current_token->type == TOKEN_IDENTIFIER) {
        ...
        // DEBUG
        printf("DEBUG: After ID '%s', next token type: %d, TOKEN_DOT is: %d\n", ad_token_kopya.value, current_token->type, TOKEN_DOT);
        fflush(stdout);
...
ASTNode* komut() {
    ...
    if (current_token->type == TOKEN_IDENTIFIER) {
        printf("DEBUG: komut() IDENTIFIER block entered. Token: %s\n", current_token->value);
...
ASTNode* kosul_komutu() {
    ...
    if (current_token->type == TOKEN_END) {
        ...
        consume(TOKEN_END);
        // Lookahead: if next token is "if", consume it ONLY if on same line
        if (current_token && current_token->type == TOKEN_IF && current_token->line == end_line) {
            consume(TOKEN_IF);
        }
    }
...
ASTNode* dongu_komutu() {
    ...
    if (current_token->type == TOKEN_END) {
        int end_line = current_token->line;
        consume(TOKEN_END);
        // Lookahead: if next token is "while", consume it ONLY if on same line
        if (current_token && current_token->type == TOKEN_WHILE && current_token->line == end_line) {
            consume(TOKEN_WHILE);
        }
    }
...
ASTNode* for_komutu() {
    ...
    if (current_token->type == TOKEN_END) {
        int end_line = current_token->line;
        consume(TOKEN_END);
        // Lookahead: if next token is "for", consume it ONLY if on same line
        if (current_token && current_token->type == TOKEN_FOR && current_token->line == end_line) {
            consume(TOKEN_FOR);
        }
    }
