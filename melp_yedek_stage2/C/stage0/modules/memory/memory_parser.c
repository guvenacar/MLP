...
MemoryAlloc* parse_memory_alloc(Lexer* lexer) {
    ...
    if (is_new) {
        ...
        MemoryAlloc* alloc = memory_alloc_create(type, count);
        free(type);
        return alloc;
        ...
        MemoryAlloc* alloc = memory_alloc_create("byte", size);
        return alloc;
    }
}
...
MemoryFree* parse_memory_free(Lexer* lexer) {
    ...
    if (is_delete) {
        ...
        MemoryFree* free_op = memory_free_create(token->value);
        token_free(token);
        return free_op;
...
MemoryCopy* parse_memory_copy(Lexer* lexer) {
    ...
    MemoryCopy* copy = memory_copy_create(dest, src, size);
    return copy;
}
...
MemoryZero* parse_memory_zero(Lexer* lexer) {
    ...
    MemoryZero* zero = memory_zero_create(var, size);
    return zero;
}
...
MemoryRealloc* parse_memory_realloc(Lexer* lexer) {
    ...
    MemoryRealloc* realloc_op = memory_realloc_create(var, new_size);
    return realloc_op;
}
