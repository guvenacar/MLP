...
// Helper: Parse and generate assembly for simple assignment statement
// Supports: var = var + number, var = var - number, var = number
static void generate_assignment(FILE *out, const char *body_str) {
    char var_name[64] = "";
    char op[8] = "";
    char right_var[64] = "";
    char operator[8] = "";
    ...
    // Try to parse: var = var + number
    int matched = sscanf(body_str, "%s = %s %s %s", var_name, right_var, operator, operand);

    if (matched == 4) {
        // Assignment with operation: counter = counter + 1
        fprintf(out, "  ; %s\n", body_str);
        fprintf(out, "  mov rax, [%s]\n", right_var);

        if (strcmp(operator, "+") == 0) {
            // Check if operand is number or variable
        ...
        // Simple assignment: var = number
        fprintf(out, "  ; %s\n", body_str);
        if (right_var[0] >= '0' && right_var[0] <= '9') {
        ...
        // Can't parse - just emit as comment
...
static void control_flow_codegen(FILE *out) {
    ...
    while (stmt != NULL) {
        ...
        // Then body - generate actual assembly instructions
        if (strlen(stmt->body) > 0) {
            generate_assignment(out, stmt->body);
        }
        fprintf(out, "  jmp .endif_%d\n", label_id);

        // Else body
        fprintf(out, ".else_%d:\n", label_id);
        if (stmt->has_else && strlen(stmt->else_body) > 0) {
            generate_assignment(out, stmt->else_body);
        }
    ...
    while (loop != NULL) {
        ...
        // Loop body - generate actual assembly instructions
        if (strlen(loop->body) > 0) {
            generate_assignment(out, loop->body);
        }
