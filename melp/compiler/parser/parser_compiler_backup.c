...
int main(int argc, char** argv) {
    ...
    // Parse
    ASTNode* root = ast_node_create(NODE_PROGRAM);
    
    while (!parser_check(&parser, TOKEN_EOF)) {
        if (parser.panic_mode) {
            parser_synchronize(&parser);
        }
        ASTNode* decl = parser_declaration(&parser);
    ...
    if (parser.had_error) {
        printf("⚠ Parser completed with errors: %s -> %s\n", input_file, output_file);
        return 1;
    }

    printf("✓ Enhanced Parser: %s -> %s\n", input_file, output_file);
    return 0;
}
