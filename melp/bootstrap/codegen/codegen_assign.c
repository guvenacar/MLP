// MELP Modular Codegen - Assignment Generation
// Atama kodu üretimi (değişken, array, struct field, pointer)
...
void codegen_generate_assignment(Codegen* gen, char* var_name, char* field_name, 
    ...
    if (dereference_target != NULL) {
        ...
        codegen_generate_pointer_assignment(gen, dereference_target, value);
    } else if (array_index != NULL) {
        // Array assignment: arr[index] = value
        codegen_generate_array_assignment(gen, var_name, array_index, value);
    } else if (field_name == NULL) {
        // Regular variable assignment
        codegen_generate_variable_assignment(gen, var_name, value);
    } else {
        // Field assignment: object.field = value
        codegen_generate_field_assignment(gen, var_name, field_name, value);
    }
}
...
void codegen_generate_pointer_assignment(Codegen* gen, Expression* dereference_target, Expression* value) {
    codegen_emit(gen, "");
...
void codegen_generate_array_assignment(Codegen* gen, char* var_name, Expression* array_index, Expression* value) {
    char buffer[256];
...
void codegen_generate_variable_assignment(Codegen* gen, char* var_name, Expression* value) {
    char buffer[256];
...
void codegen_generate_field_assignment(Codegen* gen, char* var_name, char* field_name, Expression* value) {
    char buffer[256];
    int offset = codegen_find_variable(gen, var_name);
