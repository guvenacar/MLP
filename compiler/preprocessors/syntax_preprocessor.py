...
class SyntaxPreprocessor:
    ...
    def normalize_c_style(self, source_code):
        ...
        if source_lang.startswith('tr'):
            ...
            block_start_if = 'then'
            block_start_while = ''  # while doesn't need 'then'
            block_end = 'end_if'  # Will use context-aware: end_if, end_while, end_for
        ...
        for line in lines:
            ...
            if any(kw in line_upper for kw in ['İSE', 'IF', 'EĞER', 'EGER']):
                ...
                # Don't push ELSE to stack - it's part of IF block
                pass  # context remains IF
                ...
                # Remove parentheses around condition
                line = re.sub(r'\(([^)]+)\)', r'\1', line)
            elif 'FOR' in line_upper or 'İÇİN' in line_upper or 'ICIN' in line_upper:
                context_stack.append('FOR')
            elif 'PROGRAM' in line_upper:
                context_stack.append('PROGRAM')
            elif 'FUNCTION' in line_upper or 'FONKSİYON' in line_upper:
                context_stack.append('FUNCTION')
            elif 'CLASS' in line_upper or 'SINIF' in line_upper:
                context_stack.append('CLASS')
            
            # SPECIAL CASE: } else { on the same line
            if '}' in line and 'else' in line.lower() and '{' in line:
                # Handle: } else { → else (no 'then' keyword for else in PMPL)
                line = re.sub(r'\}\s*else\s*\{', 'else', line, flags=re.IGNORECASE)
                # Don't process braces further on this line
            ...
            # IF blocks need "then/İSE", WHILE blocks don't need anything
                ...
                if context == 'IF':
                    ...
                    # WHILE blocks don't need "then" keyword in MLP, just remove the brace
            ...
            # Replace closing brace with language-appropriate block end keyword
                ...
                if source_lang.startswith('tr'):
                    ...
                    # English: context-aware (PMPL standard - underscore format)
                    if context == 'IF':
                        block_end_keyword = 'end_if'
                    elif context == 'WHILE':
                        block_end_keyword = 'end_while'
                    elif context == 'FOR':
                        block_end_keyword = 'end_for'
                    elif context == 'FUNCTION':
                        block_end_keyword = 'end_function'
                    elif context == 'PROGRAM':
                        block_end_keyword = 'end'
                    else:
                        # Default fallback - check if we're in a function
                        block_end_keyword = 'end_function' if 'function' in context.lower() else 'end'
            ...
            # - ALL semicolons should be removed in PMPL (pragmatic syntax)
            # - C-style requires them, but MLP/PMPL doesn't use them
            if ';' in line:
                line = line.replace(';', '')
...
def main():
    ...
    # Find syntax.json in the same directory as this script
