...
#include "../runtime/src/hashmap.h"  // HashMap support for function registry
...
static ImportedFunctions imported_funcs = {NULL, 0, 0};

void collect_imported_function(ASTNode* func) {
    if (imported_funcs.count >= imported_funcs.capacity) {
        imported_funcs.capacity = imported_funcs.capacity ? imported_funcs.capacity * 2 : 8;
        imported_funcs.functions = realloc(imported_funcs.functions, 
                                          sizeof(ASTNode*) * imported_funcs.capacity);
        if (!imported_funcs.functions) {
            fprintf(stderr, "HATA: Import fonksiyonları için bellek ayrılamadı\n");
            exit(1);
        }
    }
    imported_funcs.functions[imported_funcs.count++] = func;
}

void cleanup_imports() {
    if (imported_funcs.functions) {
        free(imported_funcs.functions);
        imported_funcs.functions = NULL;
        imported_funcs.count = 0;
        imported_funcs.capacity = 0;
    }
}
...
typedef enum {
    ...
    // PMLP: Exit/Continue variants (Phase 1)
...
typedef enum {
    ...
    // PMLP Phase 1: Exit/Continue variants
    AST_EXIT,               // exit (generic)
    AST_EXIT_FOR,           // exit_for
    AST_EXIT_WHILE,         // exit_while
    AST_EXIT_IF,            // exit_if
    AST_EXIT_FUNCTION,      // exit_function
    AST_EXIT_SWITCH,        // exit_switch
    AST_CONTINUE_FOR,       // continue_for
    AST_CONTINUE_WHILE,     // continue_while
...
struct ASTNode {
    ...
    union {
        ...
        struct {
            ...
            TokenType* parametre_types; // Parametre tipleri (TOKEN_TYPE_NUMERIC, TOKEN_TYPE_STRING, etc.)
...
int main(int argc, char* argv[]) {
    ...
    if (env_source_dir != NULL) {
        ...
        // This fixes import paths like "compiler_core/token_types.mlp"
    ...
    // Cleanup imported functions list
    cleanup_imports();
    
    // Cleanup function registry
    if (function_registry) {
        hashmap_destroy(function_registry);
...
KeywordMap keywords[] = {
    ...
    // PMLP: Exit/Continue keywords (Phase 1)
    {"exit",           TOKEN_EXIT},
...
Token* check_keyword(const char* word) {
    ...
    if (strcmp(word, "import") == 0) return createToken(TOKEN_IMPORT, word); // Phase 5.10: Module system
    ...
    // PMLP Phase 1: Exit/Continue variants
    if (strcmp(word, "exit") == 0) return createToken(TOKEN_EXIT, word);
    if (strcmp(word, "exit_for") == 0) return createToken(TOKEN_EXIT_FOR, word);
    if (strcmp(word, "exit_while") == 0) return createToken(TOKEN_EXIT_WHILE, word);
    if (strcmp(word, "exit_if") == 0) return createToken(TOKEN_EXIT_IF, word);
    if (strcmp(word, "exit_function") == 0) return createToken(TOKEN_EXIT_FUNCTION, word);
    if (strcmp(word, "exit_switch") == 0) return createToken(TOKEN_EXIT_SWITCH, word);
    if (strcmp(word, "continue_for") == 0) return createToken(TOKEN_CONTINUE_FOR, word);
    if (strcmp(word, "continue_while") == 0) return createToken(TOKEN_CONTINUE_WHILE, word);
    ...
    if (strcmp(word, "switch") == 0) return createToken(TOKEN_YAPI_SWITCH, word);
...
ASTNode* createAST_IslecTanimlama(Token* ad, Token** parametreler, int parametre_sayisi, TokenType* parametre_types, int* parametre_is_array, ASTNode** parametre_defaults, ASTNode* govde) {
    ASTNode* node = (ASTNode*)malloc(sizeof(ASTNode));
    if (node == NULL) return NULL;
    ...
    node->islec_tanimlama_data.parametre_types = parametre_types; // Parameter types
...
ASTNode* komut() {
    ...
    if (current_token->type == TOKEN_YAPI_STRUCT) {
        ...
        // Optional 'then' keyword (for compatibility with self-host modules)
    ...
    if (current_token->type == TOKEN_YAPI_SWITCH) {
        ...
        // Optional then keyword (for backwards compatibility)
        if (current_token->type == TOKEN_THEN) {
            consume(TOKEN_THEN);
        }
        ...
        // Accept "end switch" with lookahead (same as struct/function parsing)
        if (current_token->type == TOKEN_END) {
            consume(TOKEN_END);
    ...
    // PMLP Phase 1: exit/continue variants (Noktalı virgülsüz)
    if (current_token->type == TOKEN_EXIT) {
        consume(TOKEN_EXIT);
        specs_check_no_semicolon("exit");
        ASTNode* node = (ASTNode*)malloc(sizeof(ASTNode));
        node->type = AST_EXIT;
        return node;
    }
    if (current_token->type == TOKEN_EXIT_FOR) {
        consume(TOKEN_EXIT_FOR);
        specs_check_no_semicolon("exit_for");
        ASTNode* node = (ASTNode*)malloc(sizeof(ASTNode));
        node->type = AST_EXIT_FOR;
        return node;
    }
    if (current_token->type == TOKEN_EXIT_WHILE) {
        consume(TOKEN_EXIT_WHILE);
        specs_check_no_semicolon("exit_while");
        ASTNode* node = (ASTNode*)malloc(sizeof(ASTNode));
        node->type = AST_EXIT_WHILE;
        return node;
    }
    if (current_token->type == TOKEN_EXIT_IF) {
        consume(TOKEN_EXIT_IF);
        specs_check_no_semicolon("exit_if");
        ASTNode* node = (ASTNode*)malloc(sizeof(ASTNode));
        node->type = AST_EXIT_IF;
        return node;
    }
    if (current_token->type == TOKEN_EXIT_FUNCTION) {
        consume(TOKEN_EXIT_FUNCTION);
        specs_check_no_semicolon("exit_function");
        ASTNode* node = (ASTNode*)malloc(sizeof(ASTNode));
        node->type = AST_EXIT_FUNCTION;
        return node;
    }
    if (current_token->type == TOKEN_EXIT_SWITCH) {
        consume(TOKEN_EXIT_SWITCH);
        specs_check_no_semicolon("exit_switch");
        ASTNode* node = (ASTNode*)malloc(sizeof(ASTNode));
        node->type = AST_EXIT_SWITCH;
        return node;
    }
    if (current_token->type == TOKEN_CONTINUE_FOR) {
        consume(TOKEN_CONTINUE_FOR);
        specs_check_no_semicolon("continue_for");
        ASTNode* node = (ASTNode*)malloc(sizeof(ASTNode));
        node->type = AST_CONTINUE_FOR;
        return node;
    }
    if (current_token->type == TOKEN_CONTINUE_WHILE) {
        consume(TOKEN_CONTINUE_WHILE);
        specs_check_no_semicolon("continue_while");
        ASTNode* node = (ASTNode*)malloc(sizeof(ASTNode));
        node->type = AST_CONTINUE_WHILE;
        return node;
    }
...
ASTNode* islec_tanimlama() {
    ...
    TokenType* parametre_types = NULL; // Parameter types
    ...
    if (current_token->type != TOKEN_RIGHT_PAREN) {
        ...
        parametre_types = (TokenType*)malloc(sizeof(TokenType) * 10);
        parametre_defaults = (ASTNode**)malloc(sizeof(ASTNode*) * 10);
        parametre_is_array = (int*)malloc(sizeof(int) * 10);
        do {
            // Optional type annotation: numeric|string|boolean param_name
            TokenType param_type = TOKEN_TYPE_NUMERIC; // Default to numeric
            if (current_token->type == TOKEN_TYPE_NUMERIC || 
                current_token->type == TOKEN_TYPE_STRING || 
                current_token->type == TOKEN_TYPE_BOOLEAN) {
                // Save type annotation
                param_type = current_token->type;
                consume(current_token->type);
            }
            parametre_types[p_sayisi] = param_type;
    ...
    ASTNode* node = createAST_IslecTanimlama(&ad_token, parametre_listesi, p_sayisi, parametre_types, parametre_is_array, parametre_defaults, govde_blogu);
    free(ad_token.value);
    return node;
}
    ...
    if (current_token->type != TOKEN_RIGHT_PAREN) {
        ...
        do {
            ...
            if (current_token->type == TOKEN_TYPE_NUMERIC || 
                current_token->type == TOKEN_TYPE_STRING || 
                current_token->type == TOKEN_TYPE_BOOLEAN) {
                // Skip type annotation (just for documentation/future use)
                consume(current_token->type);
            }
...
// PMLP Phase 1: Exit/Continue variants forward declarations
void visit_Exit(ASTNode* node);
void visit_ExitFor(ASTNode* node);
void visit_ExitWhile(ASTNode* node);
void visit_ExitIf(ASTNode* node);
void visit_ExitFunction(ASTNode* node);
void visit_ExitSwitch(ASTNode* node);
void visit_ContinueFor(ASTNode* node);
void visit_ContinueWhile(ASTNode* node);
...
void visit_Import(ASTNode* node) {
    ...
    if (file_path[0] == '/') {
        ...
        // Relative path - try MLP_SOURCE_DIR first, then base_dir
        char* source_dir = getenv("MLP_SOURCE_DIR");
        if (source_dir) {
            snprintf(resolved_path, sizeof(resolved_path), "%s/%s", source_dir, file_path);
        } else if (base_dir) {
            snprintf(resolved_path, sizeof(resolved_path), "%s/%s", base_dir, file_path);
        } else {
            strncpy(resolved_path, file_path, sizeof(resolved_path) - 1);
            resolved_path[sizeof(resolved_path) - 1] = '\0';
        }
...
// PMLP Phase 1: Generic exit - exit from current loop
void visit_Exit(ASTNode* node) {
    if (aktif_dongu_son_etiketi == -1) {
        fprintf(stderr, "HATA [Generator]: 'exit' bir döngü içinde değil.\n");
        exit(1);
    }
    char buffer[128];
    sprintf(buffer, "    jmp .L%d", aktif_dongu_son_etiketi);
    asm_append(&text_section, "    ; --- Exit (generic) ---");
    asm_append(&text_section, buffer);
}

// PMLP Phase 1: exit_for - exit from for loop
void visit_ExitFor(ASTNode* node) {
    if (aktif_dongu_son_etiketi == -1) {
        fprintf(stderr, "HATA [Generator]: 'exit_for' bir for döngüsü içinde değil.\n");
        exit(1);
    }
    char buffer[128];
    sprintf(buffer, "    jmp .L%d", aktif_dongu_son_etiketi);
    asm_append(&text_section, "    ; --- Exit For ---");
    asm_append(&text_section, buffer);
}

// PMLP Phase 1: exit_while - exit from while loop
void visit_ExitWhile(ASTNode* node) {
    if (aktif_dongu_son_etiketi == -1) {
        fprintf(stderr, "HATA [Generator]: 'exit_while' bir while döngüsü içinde değil.\n");
        exit(1);
    }
    char buffer[128];
    sprintf(buffer, "    jmp .L%d", aktif_dongu_son_etiketi);
    asm_append(&text_section, "    ; --- Exit While ---");
    asm_append(&text_section, buffer);
}

// PMLP Phase 1: exit_if - exit from if block
void visit_ExitIf(ASTNode* node) {
    // exit_if jumps to end of current if block
    // For now, treat similar to exit (jump to loop end if in loop)
    if (aktif_dongu_son_etiketi == -1) {
        fprintf(stderr, "HATA [Generator]: 'exit_if' bir blok içinde değil.\n");
        exit(1);
    }
    char buffer[128];
    sprintf(buffer, "    jmp .L%d", aktif_dongu_son_etiketi);
    asm_append(&text_section, "    ; --- Exit If ---");
    asm_append(&text_section, buffer);
}

// PMLP Phase 1: exit_function - exit from function (return)
void visit_ExitFunction(ASTNode* node) {
    asm_append(&text_section, "    ; --- Exit Function ---");
    asm_append(&text_section, "    leave");
    asm_append(&text_section, "    ret");
}

// PMLP Phase 1: exit_switch - exit from switch
void visit_ExitSwitch(ASTNode* node) {
    if (aktif_dongu_son_etiketi == -1) {
        fprintf(stderr, "HATA [Generator]: 'exit_switch' bir switch içinde değil.\n");
        exit(1);
    }
    char buffer[128];
    sprintf(buffer, "    jmp .L%d", aktif_dongu_son_etiketi);
    asm_append(&text_section, "    ; --- Exit Switch ---");
    asm_append(&text_section, buffer);
}

// PMLP Phase 1: continue_for - continue in for loop
void visit_ContinueFor(ASTNode* node) {
    if (aktif_dongu_baslangic_etiketi == -1) {
        fprintf(stderr, "HATA [Generator]: 'continue_for' bir for döngüsü içinde değil.\n");
        exit(1);
    }
    char buffer[128];
    sprintf(buffer, "    jmp .L%d", aktif_dongu_baslangic_etiketi);
    asm_append(&text_section, "    ; --- Continue For ---");
    asm_append(&text_section, buffer);
}

// PMLP Phase 1: continue_while - continue in while loop
void visit_ContinueWhile(ASTNode* node) {
    if (aktif_dongu_baslangic_etiketi == -1) {
        fprintf(stderr, "HATA [Generator]: 'continue_while' bir while döngüsü içinde değil.\n");
        exit(1);
    }
    char buffer[128];
    sprintf(buffer, "    jmp .L%d", aktif_dongu_baslangic_etiketi);
    asm_append(&text_section, "    ; --- Continue While ---");
    asm_append(&text_section, buffer);
}
...
void visit_IslecTanimlama(ASTNode* node) {
    ...
    for (int i = 0; i < param_sayisi; i++) {
        ...
        TokenType param_type = TOKEN_TYPE_NUMERIC; // Default
        if (node->islec_tanimlama_data.parametre_types != NULL) {
            param_type = node->islec_tanimlama_data.parametre_types[i];
        }
        
        // Map type to string
        char* tip;
        if (is_array) {
            tip = "ARRAY_SAYISAL"; // Arrays
        } else if (param_type == TOKEN_TYPE_STRING) {
            tip = "METIN"; // String
        } else if (param_type == TOKEN_TYPE_BOOLEAN) {
            tip = "BOOLEAN"; // Boolean
        } else {
            tip = "SAYISAL"; // Numeric (default)
        }
...
void visit_StructDegisken(ASTNode* node) {
    ...
    // Built-in types için metadata gerekmez
    ...
    StructMetadata* meta = NULL;
    int type_size = 8;  // Default 8 bytes for built-ins

    if (!is_builtin) {
        // Struct metadatau0027yı bul
        meta = find_struct_metadata(struct_tip);
        if (!meta) {
            sprintf(buffer, "    ; ERROR: Struct u0027%su0027 tanımlı değil!", struct_tip);
            asm_append(&text_section, buffer);
            fprintf(stderr, "HATA [Generator]: Struct u0027%su0027 tanımlı değil!\n", struct_tip);
            return;
        }
        type_size = meta->total_size;
    }

    // Stack'te struct için yer ayır (total_size kadar)
    // Her field için offset hesaplanmış, toplamda meta->total_size byte gerekli
    kapsam_yigin_ofseti += type_size;
    char* adres = (char*)malloc(32);
    sprintf(adres, "[rbp-%d]", kapsam_yigin_ofseti);
    ...
    sprintf(buffer, "    ; Struct boyutu: %d bytes, stack adres: %s", type_size, adres);
    asm_append(&text_section, buffer);

    // Struct'ı sıfırla (opsiyonel, güvenlik için)
    sprintf(buffer, "    ; Struct memory'yi sıfırla");
    asm_append(&text_section, buffer);
    for (int offset = 0; offset < type_size; offset += 8) {
...
void visit(ASTNode* node) {
    ...
    switch (node->type) {
        ...
        case AST_EXIT_FOR:
            ...
            break;
        case AST_EXIT_WHILE:
            visit_ExitWhile(node);
            break;
        case AST_EXIT_IF:
            visit_ExitIf(node);
            break;
        case AST_EXIT_FUNCTION:
            visit_ExitFunction(node);
            break;
        case AST_EXIT_SWITCH:
            visit_ExitSwitch(node);
            break;
        case AST_CONTINUE_FOR:
            visit_ContinueFor(node);
            break;
        case AST_CONTINUE_WHILE:
            visit_ContinueWhile(node);
            break;
...
char* generate_asm(ASTNode* root) {
    ...
    if (blok->type == AST_BLOK) {
        for (int i = 0; i < blok->blok_data.sayisi; i++) {
            ...
            if (node->type == AST_IMPORT) {
                visit_Import(node);  // Collects to imported_funcs list
            }
        }
    }
    
    // ========== PHASE 2: EMIT IMPORTED FUNCTIONS (BEFORE MAIN) ==========
    if (imported_funcs.count > 0) {
        asm_append(&text_section, "\n    ; === Imported Functions ===");
        for (int i = 0; i < imported_funcs.count; i++) {
            visit(imported_funcs.functions[i]);  // ✅ NOW EMIT AT GLOBAL SCOPE
        }
        asm_append(&text_section, "    ; === End Imported Functions ===\n");
    }
    ...
    size_t total_size = data_section.size + text_section.size + stack_section_size + 50;  // +50 for safety (lambda_section already in text_section)
