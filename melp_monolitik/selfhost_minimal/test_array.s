...
func_main:
    push rbp
    mov rbp, rsp
    ; Declaration: numeric[] offsets (dynamic array, pointer)
    sub rsp, 8         ; Allocate space for pointer offsets
    mov rax, 5
    push rax
    pop rdi
    call mlp_array_alloc
    mov [rbp-8], rax   ; Store array pointer to offsets

    ; Array assignment: offsets[...] = ...
    mov rax, 0
    push rax            ; Save index
    mov rax, 8
...
section .data
str_3: db "Array test complete!", 0
str_2: db "Offset 2:", 0
str_1: db "Offset 1:", 0
str_0: db "Offset 0:", 0
